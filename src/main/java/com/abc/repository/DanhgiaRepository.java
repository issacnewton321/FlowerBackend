...
public interface DanhgiaRepository extends JpaRepository<Danhgia, DanhGia_ID> {
	...
	List<Danhgia> getDGByMakh(String masp);
	
	
	@Modifying
	@Transactional
	@Query(nativeQuery = true,value = "update danhgia set danhgia=?3 where makh = ?1 and masp =?2 ")
	void updateDanhgia(String makh,String masp,int danhgia);
}
